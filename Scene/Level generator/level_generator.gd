...
var wall_floor_dict : Dictionary #piece_connector
...
func place_corridors(curent_level_ : level,rnd_ : RandomNumberGenerator) -> void:
	var start_corridors_connectors : Array[piece_connector] = []
	for i in connectors_dict:
		var connector : piece_connector = connectors_dict[i]
		if connector.type == 1:
			start_corridors_connectors.append(connector)
	
	var corridors_connectors : Array[piece_connector] = []
	corridors_connectors = start_corridors_connectors
	
	for step_global in curent_level_.corridor_step_generate:
		
		var new_st_cor_con : Array[piece_connector] = []
		
		for connector in corridors_connectors:
			
			var direction = connector.direction
			var coord_connector = connector.coord_
			...
			for step_local in number_piece:
				...
				coord_connector = ((piece_connectors[1].coord_ + piece_connectors[2][0].coord_) + coord_new_connector)
				...
				if step_local+1 == number_piece:
					...
					if step_global +1 != curent_level_.corridor_step_generate:
						...
						coord_new_connector = DIRECTION_VECTOR[direction] + coord_connector
						offset = piece_connectors[1].coord_ - coord_new_connector
						place_piece(-offset, piece_connectors[0])
...
func place_piece(coord_piece : Vector2i, piece : level_piece):
	for i : piece_connector in piece.connectors_level:
