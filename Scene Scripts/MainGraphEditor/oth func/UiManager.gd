...
const CONNECTOR_TYPE_COUNT = 3 ## WHITE GRAY BLACK
...
func UpdateUi(cur_editor_obj_layer_id : int ,editor_obj_layer_ar : Array[EditorObjectLayer],save_data_all_library : SaveDataAllLibrary) -> void:
	...
	graph_node_ui.StartBakeUi() ## НУЖНО ДЛЯ ОЧИСТКИ И ТД
	## ХОРОШИЙ ВОПРОС КАК РАБОТАТЬ С room_set
	room_set = ResourceLoader.load(main_graph_editor.room_set_path,"",ResourceLoader.CACHE_MODE_IGNORE)
...
func bake_ui_rooms_nodes() -> void:
	for room_ind in room_set.rooms_ar.size():
		...
		if current_room != null:
			var name_item : String = ROOM_BIG_TITLE + current_room.name_+" "+str(room_ind)
...
func bake_ui_enter_node() -> void: ##Универсальный нод для обозначения входа на локу
	...
	big_instr.title_node = ENTER_LOCATION_BIG_TITLE
...
func bake_ui_connectors_plugs() -> void:
	...
	for size_ in CONNECTOR_SIZE_COUNT:
		for type_ in CONNECTOR_TYPE_COUNT:
			for direction_ in DIRECTION.size():
...
func bake_ui_start_gener_node() -> void: ##Стартовая хуйня, без нее генерация по пизде идет
	var big_instr = BigGraphNodeMakeInsts.new()
	big_instr.title_node = START_GENER_LOCATION_BIG_TITLE
...
func bake_ui_rnd_fork_from_connector( enter_connector : RoomConnector ) -> void: ## Случайные Перекрестки, Очень Круто
	...
	var big_title = BIG_RND_FORK_BIG_TITLE + " " + enter_instr.title_instr
...
func room_to_biginstrgraphnode(room : Room) -> BigGraphNodeMakeInsts:
	...
	for enter : RoomEnter in room.room_enter_ar:
		...
		instr.title_instr = " ".join([ui_const_func.TOOLENTERTITLE, BASECOLORTYPE[enter.type]]) 
